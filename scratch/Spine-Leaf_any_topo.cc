...
void ReadAndApplyRoutingTable(const std::vector<Ptr<Node>>& node_list,
    ...
    std::string routing_file = "scratch/Spine-Leaf_routes.txt";
...
int main(int argc, char *argv[]) {
    ...
    // 命令行参数
    std::string topology_file = "scratch/Spine-Leaf_topology.txt";
    CommandLine cmd;
    ...
    // 读取基本信息（Spine-Leaf格式：节点数 叶节点数 脊节点数 链路数）
    uint32_t node_num, leaf_num, spine_num, link_num;
    ...
    // 从找到的行中读取参数
    std::istringstream iss(line);
    iss >> node_num >> leaf_num >> spine_num >> link_num;
    
    // 添加调试信息
    std::ostringstream debug_oss;
    debug_oss << "读取的原始行: '" << line << "'";
    logMessage(debug_oss.str());
    
    // 验证读取的参数
    if (node_num == 0 || leaf_num == 0 || spine_num == 0 || link_num == 0) {
        std::ostringstream error_oss;
        error_oss << "错误：读取的拓扑参数无效: 节点数=" << node_num 
                  << ", 叶节点数=" << leaf_num 
                  << ", 脊节点数=" << spine_num 
                  << ", 链路数=" << link_num;
        logMessage(error_oss.str());
        std::cerr << error_oss.str() << std::endl;
        return 1;
    }
    
    std::ostringstream oss;
    oss << "拓扑参数: 节点数=" << node_num << ", 叶节点数=" << leaf_num << ", 脊节点数=" << spine_num << ", 链路数=" << link_num;
    logMessage(oss.str());
    
    // Dragonfly拓扑不需要跳过第二行，直接读取链路信息
    
    // 计算服务器数量
    uint32_t server_num = 30; // 固定30个主机节点

    {
        std::ostringstream oss;
        oss << "节点数: " << node_num << ", 主机数: " << server_num << ", 叶节点数: " << leaf_num << ", 脊节点数: " << spine_num << ", 链路数: " << link_num;
        logMessage(oss.str());
    }
    ...
    for (uint32_t i = server_num; i < node_num; i++) {
        node_types[i] = 1; // 标记为交换机（叶节点和脊节点）
        std::string node_type = (i < server_num + leaf_num) ? "叶节点" : "脊节点";
        {
            std::ostringstream oss;
            oss << "标记节点 " << i << " 为" << node_type;
            logMessage(oss.str());
        }
    }
    ...
    for (uint32_t i = 0; i < link_num; i++) {
        ...
        if (!std::getline(topof, link_line)) {
            std::ostringstream error_oss;
            error_oss << "错误：无法读取第 " << (i+1) << " 条链路信息";
            logMessage(error_oss.str());
            std::cerr << error_oss.str() << std::endl;
            return 1;
        }
        ...
        // 解析链路信息
        std::istringstream link_iss(link_line);
        if (!(link_iss >> src >> dst >> data_rate >> link_delay >> error_rate)) {
            std::ostringstream error_oss;
            error_oss << "错误：无法解析第 " << (i+1) << " 条链路信息: '" << link_line << "'";
            logMessage(error_oss.str());
            std::cerr << error_oss.str() << std::endl;
            return 1;
        }
    ...
    // 启用关键节点的数据包跟踪
    std::vector<uint32_t> trace_nodes = {0, 1, 8, 13, 20, 30, 31, 32, 33}; // 只跟踪实际存在的节点
    ...
    RunTestWithPath(0, 1, node_list, server_addresses, sink_port, 
                   "测试1: 服务器0 -> 服务器1", 
                   "服务器0和服务器1同时直连叶节点30", 1,
                   2, 2.0, 512, 3.0, 10.0);
    
    // 测试2:  - 服务器1向服务器8发送数据包 (交换机48→交换机50)
    RunTestWithPath(1, 8, node_list, server_addresses, sink_port, 
                   "测试2: 服务器1 -> 服务器8", 
                   "服务器1与交换机30直连，服务器8与交换机32直连", 2,
                   3, 3.0, 768, 15.0, 30.0);
    
    // 测试3:  - 服务器13向服务器0发送数据包
    RunTestWithPath(13, 0, node_list, server_addresses, sink_port, 
                   "测试3: 服务器13 -> 服务器0", 
                   "服务器13通过叶节点34连接到脊节点，再连接到叶节点30到达服务器0", 3,
                   4, 2.5, 1024, 35.0, 50.0);
    
    // 测试4: 服务器20向服务器29发送数据包
    RunTestWithPath(20, 29, node_list, server_addresses, sink_port, 
                   "测试4: 服务器20 -> 服务器29", 
                   "服务器20通过叶节点36连接到脊节点，再连接到叶节点39到达服务器29", 4,
                   2, 2.0, 512, 3.0, 10.0);  
    ...
    // 重点关注测试节点的统计信息
    std::vector<uint32_t> test_nodes = {0, 1, 8, 13, 20, 30}; // 测试中涉及的关键节点
    ...
    TestResult test3 = AnalyzeTestResult(0, node_list, "测试3", 13, 4096); 
    PrintTestResult(test3);
    
    // 测试4:  (服务器20 -> 服务器29)
    TestResult test4 = AnalyzeTestResult(29, node_list, "测试4", 20, 1024); // 2个包 × 512字节 = 1024字节
